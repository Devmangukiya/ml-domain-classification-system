...
export DATASET_LOC="https://raw.githubusercontent.com/GokuMohandas/Made-With-ML/main/datasets/dataset.csv"
export TRAIN_LOOP_CONFIG=configs/train_loop_config.json
...
# Get and save run ID
export RUN_ID=$(python -c "import os; from components import utils; d = utils.load_dict(os.getenv('RESULTS_FILE')); print(d['run_id'])")
...
# Save to S3 
export MODEL_REGISTRY=$(python -c "from components import config; print(config.MODEL_REGISTRY)")
BUCKET=model-registry-dev-mangukiya-v2


GITHUB_USERNAME=Devmangukiya
aws s3 cp "$MODEL_REGISTRY" \
  "s3://$BUCKET/$GITHUB_USERNAME/mlflow_1/" \
  --recursive

aws s3 cp results/ \
  "s3://$BUCKET/$GITHUB_USERNAME/results_1/" \
  --recursive


echo "Run succesfully completed"